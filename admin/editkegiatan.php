...
mysqli_query($connection, "UPDATE kegiatan set eventNAMA='$eventNAMA',
				  eventKET='$eventKET',eventMULAI='$eventMULAI',
				  eventSELESAI='$eventSELESAI', kabupatenKODE='$kabupatenKODE', eventPOSTER='$nama' 
				  WHERE eventKODE='$eventKODE'");
	header("location:kegiatan.php"); 
}
$eventkode = $_GET["kegiatanubah"];
$edit = mysqli_query($connection, "SELECT * FROM kegiatan WHERE eventKODE = '$eventkode'");
...
<form method="POST" enctype="multipart/form-data">
  ...
  id="eventKODE" name="inputkodeevent" value="<?php echo $row_edit["eventKODE"]?>"/>
    </div>
  </div>
...
<div class="form-group row">
    ...
    <div class="col-sm-9">
      <input type="text" class="form-control" 
  id="eventNAMA" name="inputnamaevent" value="<?php echo $row_edit["eventNAMA"]?>"/>
    </div>
  </div>
...
<div class="form-group row">
    ...
    <div class="col-sm-9">
      <input type="text" class="form-control" 
  id="eventKET" name="inputketevent" value="<?php echo $row_edit["eventKET"]?>"/>
    </div>
  </div>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-9">
	  ...
	  <img src="images/<?php echo $row_edit['eventPOSTER']?>" style="width:200px;height:100px">
	  <p class="help-block">Field ini digunakan untuk mengambil file gambar/foto Kegiatan</p>
    </div>
  </div>
...
<div class="form-group row">
    <label for="datepicker" class="col-sm-3 col-form-label">Tanggal Event Mulai</label>
    <div class="col-sm-9">
      <input type="text" class="form-control" 
  id="datepicker" name="inputeventmulai" value="<?php echo $row_edit["eventMULAI"]?>"/>
    </div>
  </div>

<div class="form-group row">
    <label for="datepicker" class="col-sm-3 col-form-label">Tanggal Event Selesai</label>
    <div class="col-sm-9">
      <input type="text" class="form-control" 
  id="datepicker" name="inputeventselesai" value="<?php echo $row_edit["eventSELESAI"]?>"/>
    </div>
  </div>
   ...
   while ($row = mysqli_fetch_array ($query)) { ?> 
    <tr>
	  ...
	  <td>
				...
				<a href="editkegiatan.php?kegiatanubah=<?php echo $row["eventKODE"]?>">   Update</a> |
				<a href="javascript:;"><i class="fa fa-pencil-square-o"></i>
				<a href="hapuskegiatan.php?kegiatanhapus=<?php echo $row["eventKODE"]?>">   Delete</a>
	  </td>
	  ...
	  <td>
	  <?php if($row['eventPOSTER']=="") {echo "<img src='images/noimage.png' width='100'/>";}
	  else {?>
